{%- macro ds_and_dim_initial_create_sql(target_relation, temp_relation, change_key) -%}
    ...
    {% if change_key != 'unknown'  %}
     -- Type 2 solution
      SELECT 
        {{ dbt_utils.surrogate_key(business_key_list + change_key_list ) }} as {{temp_relation.name}}_KEY, 
        ,*
    ...
    -- Type 1 solution
      SELECT
        {{ dbt_utils.surrogate_key(business_key_list) }} as {{temp_relation.name}}_KEY 
        , *
        ,{{ CurrentTimeStamp }} as DSS_CREATE_DATE
        ,{{ CurrentTimeStamp }} as DSS_UPDATE_DATE
      FROM
